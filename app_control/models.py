...
class Shop(models.Model):
    created_by=models.ForeignKey(CustomUser, null=True, related_name="shops",
    on_delete=models.SET_NULL)
    ...
    def __init__(self, *args,**kwargs):
        ...
        self.old_name = self.name    
    def save(self, *args, **kwargs):
        action = f"add new shop - '{self.name}'"
        if self.pk is not None:
...
class Invoice(models.Model):
    created_by=models.ForeignKey(CustomUser, null=True, related_name="invoices",
    on_delete=models.SET_NULL)
...
class InvoiceItem(models.Model):
    invoice = models.ForeignKey(Invoice,related_name="invoice_itens",on_delete=models.CASCADE)
    ...
    def save(self,*args,**kwargs):
        ...
        self.item.remaining =self.item.remainig - self.quantity 
        self.item.save()
